...
describe('Ao chamar o controller de findMovieById', () => {
  describe('quando não existem filmes no banco de dados', async () => {
    ...
    before(() => {
      ...
      sinon.stub(MovieService, 'findMovieById')
        .resolves(null);
    })

    after(() => {
      MovieService.findMovieById.restore();
    })

    it('é chamado o método "status" passando 404', async () => {
      await MovieController.findMovieById(request, response);
